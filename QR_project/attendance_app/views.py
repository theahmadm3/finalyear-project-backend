...
class CreateStudentAttendance(APIView):
    ...
    def process_attendance(self, lecture, request_user,location,course_id):
        try:
            recent_attendance = StudentAttendance.objects.filter(lecture=lecture, lecturer=request_user).latest('timestamp').timestamp
            ...
            if self.checkWithinInterval(recent_attendance, time_frame):
                message = "You have already Recorded your attendance"
            else:
                lecturer_attendance = StudentAttendance.objects.create(lecture=lecture, lecturer=request_user)
